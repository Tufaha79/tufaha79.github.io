...
document.addEventListener("DOMContentLoaded", () => {
    ...
    age_enter.addEventListener("keypress", (event) => {
        age = age_enter.value
        if (event.which === KEY_ENTER && age >= 1) {
            ...
            for (let a = 0; a < 5; a++) {
                ...
                input.addEventListener("keypress", (event) => {
                    ...
                    if (event.which === KEY_ENTER) {
                        ...
                        let input_id = parseInt(input.id)
                        if (sol === task.solution) {
                            input.parentElement.parentElement.style.display = "none"

                          next_input_focus(input_id, tasks)
    ...
    function next_input_focus(input_id, tasks, next_input) {

      next_input = null
      let next_input_id = (input_id + 1) % tasks.length
      while (next_input == null) {
          if (document.getElementById(next_input_id).parentElement.parentElement.style.display !== "none") {
              next_input = document.getElementById(next_input_id)
          }
          next_input_id = (next_input_id + 1) % tasks.length
      }
